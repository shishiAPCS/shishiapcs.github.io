...
| V02 | **void method** / `void` 方法、无返回值方法 | A method that does not return a value. Its method call is not used as part of an expression. **Example:** `public static void printMessage()` |
| V03 | **non-void method** / 非 `void` 方法、有返回值方法 | A method that calculates and returns a value of a specific type. **Example:** `square(5)` returns `25`. |
| V04 | **return type** / 返回类型 | The data type of the value a method returns. It appears before the method name in the header. **Example:** `int` in `public static int square(int number)` |
| V05 | **return value** / 返回值 | The actual value sent back to the calling code by a non-void method. **Example:** `square(5)` returns `25`. |
| V06 | **return statement** / `return` 语句 | Sends a value back to the calling code and ends the method. **Example:** `return number * number;` |
| V07 | **using a return value** / 使用返回值 | A return value can be stored in a variable or used directly in an expression. **Example:** `int y = square(5);` or `System.out.println(square(4));` |
| V08 | **method-call compatibility** / 方法调用匹配 | Arguments must match the method's parameters in number, order, and compatible types. The returned value must also be stored in a compatible type. **Example:** `double result = divide(5, 2);` |
| V09 | **class method call** / 类方法调用、静态方法调用 | A class method from another class is typically called using `ClassName.methodName(arguments)`. The dot `.` connects the class and method. **Example:** `MathFunctions.square(3)` |
