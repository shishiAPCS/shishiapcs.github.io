...
### 2026-09-15 更新：自动模型安装与中英文界面

这次更新主要降低首次使用门槛，让学生打开 SpeakDeck 后可以更快直接开始使用。

主要变化：

* **Whisper 模型自动安装**：Transcribe 首次打开时，如果浏览器中没有已有模型，会自动下载默认的 `tiny.en`；
* **模型自动缓存**：下载完成后保存到浏览器 IndexedDB，之后打开通常可以直接从缓存恢复，无需重复下载；
* **保留手动模型功能**：仍然可以手动导入 `tiny.en`、`base.en`、`small.en` 等模型，手动选择不会被默认模型覆盖；
* **新增下载进度显示**：首次下载 Whisper 模型时，会在「模型状态」中显示进度条、百分比和已下载大小；
* **默认简体中文界面**：Transcribe 和 TTS 两个页面均默认使用中文，更适合学生直接使用；
* **新增中英文切换**：`SpeakDeck` 标题旁增加 `EN / 中文` 切换按钮，两个页面共用同一个语言设置，并自动记住用户的选择；
* **运行状态同步双语化**：模型下载、音频准备、转写、TTS 生成等动态状态也会随界面语言一起切换；
* Whisper、Kokoro、模型名称、WAV 以及英语练习文本等技术名称和学习内容继续保留英文。
