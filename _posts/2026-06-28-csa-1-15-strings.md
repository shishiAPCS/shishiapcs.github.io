...
# Unit 1.15 Vocabulary
## Strings

| Card | Vocabulary | Meaning & Example |
| --- | --- | --- |
| V01 | **String / String object** / 字符串、字符串对象 | A `String` represents a sequence of characters used to store text. In Java, `String` is a class, so strings are objects. **Example:** `String name = "Alice";` |
| V02 | **string literal** / 字符串字面量 | A sequence of characters enclosed in double quotes that creates a String object. **Example:** `"Hello"` |
| V03 | **concatenation** / 字符串连接、拼接 | Combining strings using `+` or `+=` to create a new String. Other values can also be converted to strings during concatenation. **Example:** `"Hello " + name` |
| V04 | **immutable** / 不可变的 | A String object cannot be changed after it is created. Operations that seem to modify a String actually create a new String. **Example:** `s = s + "!";` creates a new String and assigns it to `s`. |
| V05 | **index** / 索引 | The position of a character in a String. String indices start at `0`, so the last valid index is `length() - 1`. **Example:** in `"cat"`, `c` is at index `0`. |
| V06 | **length()** / `length()`、字符串长度方法 | Returns the number of characters in a String, including spaces and punctuation. **Example:** `"baby".length()` returns `4`. |
| V07 | **substring()** / `substring()`、子字符串方法 | Returns a new String containing part of the original String. The starting index is included, but the ending index is excluded. **Example:** `"baby".substring(0, 3)` returns `"bab"`. |
| V08 | **indexOf()** / `indexOf()`、查找位置方法 | Searches for a String and returns the index of its **first occurrence**. Returns `-1` if it is not found. **Example:** `"abccba".indexOf("b")` returns `1`. |
| V09 | **equals()** / `equals()`、字符串相等比较 | Compares the characters in two Strings and returns `true` if they are the same. String comparison is case-sensitive. **Example:** `"Hi".equals("Hi")` is `true`, but `"Hi".equals("hi")` is `false`. |
| V10 | **compareTo()** / `compareTo()`、字符串顺序比较 | Compares two Strings alphabetically. It returns a negative value if the first comes before the second, `0` if they are equal, and a positive value if the first comes after the second. **Example:** `"Apple".compareTo("Zoo")` returns a negative value. |
| V11 | **IndexOutOfBoundsException** / 索引越界异常 | An error caused by trying to use an index outside the valid range of a String. Valid indices are from `0` through `length() - 1`. **Example:** `"cat".substring(5)` causes an `IndexOutOfBoundsException`. |
| V12 | **toString()** / `toString()`、字符串表示方法 | Returns a String representation of an object. When an object is concatenated with a String, Java implicitly uses the object's `toString()` method. **Example:** `"Object: " + obj` converts `obj` to a String representation. |
