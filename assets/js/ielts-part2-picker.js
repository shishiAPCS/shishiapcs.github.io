(() => {
      ...
      document.querySelectorAll(selector).forEach((candidate) => {
        const count = [
          ...candidate.querySelectorAll(
            'h1,h2,h3,h4,h5,h6,p,strong,.topic-subheading'
          )
  ...
  function parseTopics() {
    ...
    const blocks = [
      ...root.querySelectorAll(
        'h1,h2,h3,h4,h5,h6,p,ul,ol,summary,.topic-subheading'
      )
    ];
    ...
    for (let i = 0; i < blocks.length; i += 1) {
      ...
      for (let p = i - 1; p >= 0; p -= 1) {
        ...
        if (
          /^(Part 1|Part 2|Part 3|You should say:?)$/i.test(candidate)
        ) {
          continue;
        }

        if (/^Practice wording standardized/i.test(candidate)) {
          continue;
        }
        ...
        if (
          /^(一、?新题|二、?保留题|三、?必考题|四、?补充报告题)/u.test(
            candidate
          )
        ) {
          break;
      ...
      for (let j = i + 1; j < blocks.length; j += 1) {
        ...
        if (
          candidateText === 'Part 3' ||
          candidateText === 'Part 2'
        ) {
          break;
        }
        ...
        if (
          foundCueLabel &&
          /^(UL|OL)$/.test(candidateBlock.tagName)
        ) {
          cues = [
            ...candidateBlock.querySelectorAll(':scope > li')
          ]
            .map((li) => exactText(li))
            .filter(Boolean);
      ...
      if (!title || !question || cues.length === 0) {
        continue;
      }

      const id = hashString(
        `${section}|${category}|${title}|${question}`
      );
  ...
  function loadState() {
    try {
      ...
      return {
        ...
        statusFilter:
          ['all', 'new', 'retained'].includes(state.statusFilter)
            ? state.statusFilter
            : 'all',

        categoryFilter:
          ['all', 'person', 'place', 'thing', 'event'].includes(
  ...
  if (!topics.length) {
    console.warn(
      '[IELTS Part 2 Picker] No Part 2 topics found on this page.'
    );
    return;
  }
  ...
  state.used = state.used.filter((id) =>
    validIds.has(id)
  );
  ...
  if (!validIds.has(state.currentId)) {
  ...
  function saveState() {
    try {
      ...
      // Picker still works without localStorage.
    }
  }
  ...
  style.textContent = `
    ...
    #ielts-part2-launcher:hover {
      transform: translateY(-2px);
      background: var(--ielts-panel-2);
    }

    #ielts-part2-launcher:focus-visible,
    .ielts-picker button:focus-visible {
      outline:
        3px solid var(--ielts-accent);
    ...
    .ielts-picker__brand strong {
      font-size:
        clamp(18px,2vw,24px);
    }

    .ielts-picker__brand span {
    ...
    .ielts-picker__btn:hover {
      filter: brightness(1.1);
    }

    .ielts-picker__btn:disabled {
      opacity: .42;
      cursor: default;
      filter: none;
    }
    ...
    .ielts-picker__time.is-low {
      color:
        var(--ielts-warn);
    }

    .ielts-picker__time.is-done {
      color:
        var(--ielts-danger);
    }
    ...
    .ielts-picker__toast.is-visible {
      opacity: 1;
  ...
  modal.setAttribute(
    'aria-label',
    'IELTS Part 2 random topic picker'
  );
  ...
  modal.innerHTML = `
    ...
    <div
      class="ielts-picker__filters"
      aria-label="Topic filters"
    >

      <div
        class="ielts-picker__filter-group"
        data-filter-group="status"
      >
        <button
          class="ielts-picker__filter"
          type="button"
          data-status="all"
        >
          All
    ...
    <div class="ielts-picker__stage">
      <article
        class="ielts-picker__card"
        aria-live="polite"
      >
        <div class="ielts-picker__meta">
          <span
            class="ielts-picker__chip"
    ...
    <div class="ielts-picker__controls">
      ...
      <div class="ielts-picker__right">
        ...
        <div
          class="ielts-picker__timer"
          aria-label="
            One minute preparation timer
          "
        >
          <span
            class="ielts-picker__time"
            data-role="time"
          >
            1:00
          </span>
  ...
  function remainingTopics() {
    ...
    return matchingTopics().filter(
      (topic) => !used.has(topic.id)
    );
  }
  ...
  function renderTopic() {
    ...
    if (!topic) {
      ...
      els.remaining.textContent =
        pool.length
          ? `${remaining.length} of ${pool.length} available`
          : 'No matching topics';
      ...
      els.topicContent.innerHTML = `
        <div class="ielts-picker__empty">
          ...
          <span>
            ${
              pool.length
                ? 'Press “Pick next topic” to begin.'
                : 'Try another filter.'
            }
          </span>
        </div>
    ...
    els.statusChip.classList.toggle(
      'ielts-picker__chip--retained',
      topic.section === 'retained'
    );

    els.categoryChip.textContent =
      CATEGORY_LABELS[
        topic.category
      ] || topic.category;

    els.categoryChip.classList.remove(
      'ielts-picker__chip--retained'
    );

    els.remaining.textContent =
      `${remaining.length} of ` +
      `${pool.length} left with current filters`;

    els.topicContent.innerHTML = `
      <h2 class="ielts-picker__cn-title">
        ${escapeHtml(topic.title)}
      </h2>

      <p class="ielts-picker__question">
        ${escapeHtml(topic.question)}
      </p>

      <p class="ielts-picker__cue-label">
        You should say:
      </p>
  ...
  function pickNext() {
    ...
    if (!available.length) {
      ...
      showToast(
        'All matching topics have been used. ' +
        'Reset used topics to start again.'
      );
